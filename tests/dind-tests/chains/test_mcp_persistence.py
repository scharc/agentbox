...
class TestMCPPersistence:
    ...
    def test_mcp_remove_cleans_config(self, test_project):
        ...
        # Add MCP first
        run_abox("mcp", "add", "sqlite", cwd=test_project)

        mcp_file = test_project / ".agentbox" / "claude" / "mcp.json"
        with open(mcp_file) as f:
            config = json.load(f)
        assert "sqlite" in config.get("mcpServers", {})

        # Remove MCP
        result = run_abox("mcp", "remove", "sqlite", cwd=test_project)
        assert result.returncode == 0

        # Verify removed from config
        with open(mcp_file) as f:
            config_after = json.load(f)
        assert "sqlite" not in config_after.get("mcpServers", {})
...
class TestMCPInWorktree:
    ...
    def test_mcp_available_in_worktree(self, test_project, fake_git_repo):
        ...
        # 3. Create worktree using agentbox command
