...
class TestBaseImageBuildArgs:
    ...
    def test_agentbox_version_available(self, running_container, test_project):
        """Test that AGENTBOX_VERSION is available."""
        container_name = f"agentbox-{test_project.name}"

        result = exec_in_container(
            container_name,
            "python3 -c 'from agentbox import __version__; print(__version__)'"
        )
    ...
    def test_base_image_constant(self, running_container, test_project):
        ...
        result = exec_in_container(
            container_name,
            "python3 -c 'from agentbox.container import ContainerManager; "
...
class TestBaseImageList:
    ...
    def test_docker_images_list(self, running_container, test_project):
        ...
        result = exec_in_container(
            container_name,
            "docker images --format '{{.Repository}}:{{.Tag}}' | grep agentbox-base || echo 'NO_BASE_IMAGE'"
        )

        assert result.returncode == 0
        # Either finds base image or reports none found
        assert "agentbox-base" in result.stdout or "NO_BASE_IMAGE" in result.stdout

    def test_base_image_inspection(self, running_container, test_project):
        """Test inspecting base image if it exists."""
        container_name = f"agentbox-{test_project.name}"

        # Try to inspect agentbox-base:latest
        result = exec_in_container(
            container_name,
            "docker image inspect agentbox-base:latest --format '{{.Id}}' 2>&1 || echo 'NOT_FOUND'"
        )
...
class TestBaseImageVerification:
    ...
    def test_current_container_image(self, running_container, test_project):
        ...
        assert "agentbox" in result.stdout or "INSPECT_FAILED" in result.stdout
    ...
    def test_base_image_layers(self, running_container, test_project):
        ...
        result = exec_in_container(
            container_name,
            "docker image history agentbox-base:latest --format '{{.CreatedBy}}' 2>&1 | head -5 || echo 'NOT_FOUND'"
        )
...
class TestBaseImageIntegration:
    ...
    def test_check_base_image_exists(self, running_container, test_project):
        ...
        result = exec_in_container(
            container_name,
            "docker image inspect agentbox-base:latest >/dev/null 2>&1 && echo 'EXISTS' || echo 'NOT_FOUND'"
        )
    ...
    def test_base_image_size(self, running_container, test_project):
        ...
        result = exec_in_container(
            container_name,
            "docker image inspect agentbox-base:latest --format '{{.Size}}' 2>&1 || echo 'NOT_FOUND'"
        )
    ...
    def test_base_image_created_date(self, running_container, test_project):
        ...
        result = exec_in_container(
            container_name,
            "docker image inspect agentbox-base:latest --format '{{.Created}}' 2>&1 || echo 'NOT_FOUND'"
        )
...
class TestBaseImageTagging:
    ...
    def test_image_tags_list(self, running_container, test_project):
        """Test listing all agentbox-related images."""
        container_name = f"agentbox-{test_project.name}"

        result = exec_in_container(
            container_name,
            "docker images | grep agentbox | awk '{print $1\":\"$2}' || echo 'NO_IMAGES'"
        )
    ...
    def test_base_image_latest_tag(self, running_container, test_project):
        ...
        result = exec_in_container(
            container_name,
            "docker images agentbox-base:latest -q 2>&1 || echo 'NOT_FOUND'"
        )
...
class TestVersionTracking:
    """Test agentbox version tracking in project config."""
...
with tempfile.TemporaryDirectory() as tmpdir:
    config_path = Path(tmpdir) / ".agentbox.yml"
    config = ProjectConfig(config_path)

    # Initially None
    print(f"INITIAL:{config.agentbox_version}")

    # Set version
    config.agentbox_version = "1.2.3"
    print(f"AFTER_SET:{config.agentbox_version}")
"""
    ...
    def test_is_version_outdated_same_version(self, running_container, test_project):
        ...
        script = """
from agentbox.config import ProjectConfig
from agentbox import __version__ as CURRENT_VERSION
from pathlib import Path
import tempfile
...
with tempfile.TemporaryDirectory() as tmpdir:
    ...
    config.agentbox_version = CURRENT_VERSION
...
with tempfile.TemporaryDirectory() as tmpdir:
    ...
    print(f"VERSION_IN_FILE:{data.get('agentbox_version')}")
"""
...
class TestOutdatedBaseImageDetection:
    ...
    def test_is_base_image_outdated_method_exists(self, running_container, test_project):
        ...
        result = exec_in_container(
            ...
            "python3 -c 'from agentbox.container import ContainerManager; "
            "m = ContainerManager(); "
            "print(f\"HAS_METHOD:{hasattr(m, \\\"is_base_image_outdated\\\")}\")'"
        )
