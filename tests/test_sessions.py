"""Tests for agentbox/sessions.py (and agentbox.core.sessions)"""
...
class TestGetSessionsForContainer:
    ...
    @patch('agentbox.container.ContainerManager')
    @patch('agentbox.core.sessions.list_tmux_sessions')
    def test_get_sessions_running_container(self, mock_list_tmux, mock_manager_class):
        ...
        result = get_sessions_for_container("agentbox-test")

        assert result == mock_sessions
        mock_manager.is_running.assert_called_once_with("agentbox-test")
        mock_list_tmux.assert_called_once_with(mock_manager, "agentbox-test")
...
class TestGetAllSessions:
    ...
    def test_get_all_sessions_multiple_containers(self, mock_list_tmux, mock_manager_class):
        ...
        mock_container1.name = "agentbox-project1"
        mock_container2 = Mock()
        mock_container2.name = "agentbox-project2"
        ...
        def mock_sessions(manager, container_name):
            if container_name == "agentbox-project1":
                return [{"name": "claude", "windows": 1, "attached": False, "created": "2024-01-01"}]
            elif container_name == "agentbox-project2":
        ...
        assert len(result) == 2
        assert result[0]["container"] == "agentbox-project1"
        assert result[0]["name"] == "claude"
        assert result[1]["container"] == "agentbox-project2"
    ...
    def test_get_all_sessions_filters_non_agentbox(self, mock_list_tmux, mock_manager_class):
        """Test that non-agentbox containers are filtered out"""
        mock_manager = Mock()
        mock_manager_class.return_value = mock_manager
        ...
        # Should only get sessions from agentbox container
        assert len(result) == 1
        assert result[0]["container"] == "agentbox-project1"
...
class TestCaptureSessionOutput:
    ...
    @patch('agentbox.container.ContainerManager')
    @patch('agentbox.core.sessions.capture_pane')
    def test_capture_output_success(self, mock_capture, mock_manager_class):
        ...
        result = capture_session_output("agentbox-test", "claude", lines=50)

        assert result == "Test output\nLine 2\nLine 3"
        mock_capture.assert_called_once_with(mock_manager, "agentbox-test", "claude", 50)
    ...
    def test_capture_output_stopped_container(self, mock_manager_class):
        ...
        result = capture_session_output("agentbox-test", "claude")
...
class TestSendKeysToSession:
    ...
    @patch('agentbox.container.ContainerManager')
    @patch('agentbox.core.sessions.send_keys')
    def test_send_keys_success(self, mock_send, mock_manager_class):
        ...
        result = send_keys_to_session("agentbox-test", "claude", "ls\n")

        assert result is True
        mock_send.assert_called_once_with(mock_manager, "agentbox-test", "claude", "ls\n", True)
    ...
    def test_send_keys_stopped_container(self, mock_manager_class):
        ...
        result = send_keys_to_session("agentbox-test", "claude", "test")
...
class TestResizeSession:
    ...
    @patch('agentbox.container.ContainerManager')
    @patch('agentbox.core.sessions.resize_window')
    def test_resize_success(self, mock_resize, mock_manager_class):
        ...
        result = resize_session("agentbox-test", "claude", 80, 24)

        assert result is True
        mock_resize.assert_called_once_with(mock_manager, "agentbox-test", "claude", 80, 24)
