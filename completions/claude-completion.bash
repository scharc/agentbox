...
# Usage: source completions/claude-completion.bash

_claude_completion_commands() {
    local cmd="$1"
...
_claude_completion() {
    ...
    if [[ "$cur" == -* ]]; then
        mapfile -t completions < <(_claude_completion_options "$cmd")
    elif [[ ${COMP_CWORD} -le 1 || "$prev" == "help" ]]; then
        mapfile -t completions < <(_claude_completion_commands "$cmd")
    fi
