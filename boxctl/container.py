...
"""Container lifecycle management for boxctl."""
...
from boxctl.utils.exceptions import ConfigError
from boxctl import container_naming
from boxctl.paths import ContainerPaths, ContainerDefaults, HostPaths, ProjectPaths
import threading
...
_CONTAINER_CACHE_TTL = float(os.environ.get("BOXCTL_CONTAINER_CACHE_TTL", "2.0"))
...
def get_abox_environment(include_tmux: bool = False, container_name: str = None) -> dict:
    ...
    Args:
        ...
        container_name: If provided, includes BOXCTL_CONTAINER_NAME for MCP servers

    Returns:
        Dict with standard environment variables
    """
    env = {"HOME": ContainerPaths.HOME, "USER": ContainerPaths.USER}
    if include_tmux:
        env["TMUX_TMPDIR"] = "/tmp"
    if container_name:
        env["BOXCTL_CONTAINER_NAME"] = container_name
    return env


class ContainerManager:
    """Manages boxctl Docker containers."""

    # Use centralized constants from ContainerDefaults
    BASE_IMAGE = ContainerDefaults.BASE_IMAGE
    ...
    def get_runtime_dir(self, project_name: str) -> Path:
        ...
        """
        return self.BOXCTL_DIR / "runtime" / project_name
    ...
    def is_base_image_outdated(self, container_name: str) -> bool:
        ...
        Compares the container's image ID with the current boxctl-base:latest image.
    ...
    def create_container(
        ...
        # Ensure .boxctl/state directory exists in project
        # Note: state is no longer mounted when host ~/.claude is mounted directly.
        boxctl_dir = ProjectPaths.boxctl_dir(project_dir)
        state_dir = boxctl_dir / "state"
        ...
        boxctld_dir = str(HostPaths.boxctld_dir())
        ...
        # These are at ~/.config/boxctl/{mcp,skills}/ on host, mounted to /home/abox/.config/boxctl/
        ...
        # Mount boxctld socket directory (for streaming/notifications)
        # Create dir if needed - socket will appear when service starts
        Path(boxctld_dir).mkdir(parents=True, exist_ok=True)
        volumes[boxctld_dir] = {"bind": boxctld_dir, "mode": "ro"}
        ...
        # In tunnel mode, ports are exposed dynamically via boxctld
        ...
        try:
            ...
            # Restore container network connections from .boxctl/config.yml
    ...
    def print_containers_table(self, all_containers: bool = False) -> None:
        ...
        table = Table(title="boxctl Containers")
    ...
    def get_all_containers(self, include_boxctl: bool = False) -> List[Dict[str, Any]]:
        """List all running Docker containers.

        Args:
            include_boxctl: Include boxctl containers in results
        ...
        result = []
        for container in containers:
            # Skip boxctl containers unless requested
            if not include_boxctl and container.name.startswith(self.CONTAINER_PREFIX):
                continue
