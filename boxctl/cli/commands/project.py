...
@handle_errors
def init():
    """Initialize .boxctl/ directory structure (non-interactive).

    Creates the .boxctl/ directory with default configuration files.
    Use 'boxctl setup' for an interactive setup wizard that helps
    ...
    if boxctl_dir.exists():
        console.print(f"[yellow].boxctl/ already initialized in {project_dir}[/yellow]")

    console.print(f"[green]Initializing .boxctl/ in {project_dir}...[/green]")

    # Create directory structure
    # Agent configs are created in home dirs at container startup
    # Only project-level files go in .boxctl/
    for subdir in ["skills", "mcp"]:
        (boxctl_dir / subdir).mkdir(parents=True, exist_ok=True)
    ...
    if lib_manager.skills_dir.exists():
        for skill_path in lib_manager.skills_dir.iterdir():
            if skill_path.is_dir():
                target_path = boxctl_dir / "skills" / skill_path.name
    ...
    for src_rel, dest_rel in template_copies:
        ...
        dest = boxctl_dir / dest_rel
        if src.exists() and not dest.exists():
            dest.write_text(src.read_text())

    # Workspaces config
    workspaces_path = boxctl_dir / "workspaces.json"
    if not workspaces_path.exists():
        workspaces_path.write_text(json.dumps({"workspaces": []}, indent=2))

    # Gitignore
    gitignore_path = boxctl_dir / ".gitignore"
...
"""
    ...
    # Log file
    log_path = boxctl_dir / LOG_DOC_NAME
    if not log_path.exists():
        log_path.write_text("# boxctl Log\n\n- ")

    # Add default MCPs (agentctl, boxctl-analyst)
    from boxctl.cli.commands.mcp import _add_mcp
    ...
    init_pctx = _InitPctx(boxctl_dir, project_dir)
    default_mcps = ["agentctl", "boxctl-analyst"]
    ...
    console.print("\n[green]✓ Initialized .boxctl/[/green]")
    console.print("\n[blue]Created:[/blue]")
    console.print("  .boxctl/config.yml (project config)")
    console.print("  .boxctl/agents.md (agent instructions)")
    console.print("  .boxctl/superagents.md (super agent instructions)")
    console.print("  .boxctl/mcp/ (MCP server code)")
    console.print("  .boxctl/mcp-meta.json (MCP tracking)")
...
def _send_test_notification(project_name: str) -> None:
    """Send a test notification through boxctld."""
    import json
    import socket
    ...
    if not sock_path.exists():
        console.print("[yellow]⚠ boxctld not running - start with 'boxctl service start'[/yellow]")
        return

    payload = {
        "action": "notify",
        "title": f"{project_name} | Test",
        "message": "This is a test notification from boxctl reconfigure",
        "urgency": "normal",
    }
...
def reconfigure():
    ...
    and project settings (.boxctl/config.yml).
    """
    project_dir = resolve_project_dir()
    boxctl_dir = get_boxctl_dir(project_dir)

    if not boxctl_dir.exists():
        console.print("[red]Error: .boxctl/ not found. Run 'boxctl init' first.[/red]")
        sys.exit(1)

    console.print(BANNER, highlight=False, markup=False)
    console.print("[bold cyan]boxctl Configuration[/bold cyan]")
    ...
    config_dir = boxctl_dir / "config" / "claude"
    ...
    if host_config.socket_path.exists():
        ...
        console.print(f"\n[dim]Notifications require 'boxctl service start' (optional)[/dim]")
...
@project.command(options_metavar="")
def setup():
    """Initialize and configure boxctl for this project.

    Runs 'init' to create .boxctl/ directory, then 'reconfigure'
...
def start():
    ...
    if pctx.boxctl_dir.exists():
        console.print("[blue]Syncing MCP servers from library...[/blue]")
        _sync_library_mcps(pctx.boxctl_dir, quiet=True)
    ...
    console.print("  boxctl shell    - Enter interactive shell")
    console.print("  boxctl claude   - Run Claude Code")
    console.print("  boxctl codex    - Run Codex")
    console.print("  boxctl info     - Get container details")
...
def info(project_name: Optional[str]):
    ...
    config = ProjectConfig(pctx.project_dir)
    if not config.exists():
        console.print(f"\n[dim]No config file found at {pctx.project_dir}/.boxctl/config.yml[/dim]")
        return
...
def rebase(scope: Optional[str]):
    ...
    Recreates the container from scratch using the latest boxctl-base:latest.
...
def _rebase_all_containers(manager: ContainerManager) -> None:
    """Rebase all existing boxctl containers to the current base image."""
...
def config_migrate(dry_run: bool, auto: bool):
    ...
    if dry_run:
        from boxctl.migrations.base import MigrationAction
