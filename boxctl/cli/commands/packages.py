...
import boxctl.cli as cli_module
...
def packages_list():
    ...
    _require_boxctl_dir(boxctl_dir, project_dir)

    packages = _load_packages_config(boxctl_dir)
    ...
    if not has_packages:
        ...
        console.print("  boxctl packages add npm <package>")
        console.print("  boxctl packages add pip <package>")
        console.print("  boxctl packages add apt <package>")
        console.print("  boxctl packages add cargo <package>")
        return
    ...
    console.print(table)
    console.print("\n[dim]Packages are installed when you add them or run 'boxctl rebase'[/dim]")
...
def packages_add(package_type: str, package: str):
    ...
    Examples:
      ...
      boxctl packages add pip black
      boxctl packages add apt neovim
      boxctl packages add cargo ripgrep
      boxctl packages add post "curl -sSL https://example.com/install.sh | bash"
    """
    project_dir = resolve_project_dir()
    boxctl_dir = get_boxctl_dir(project_dir)
    _require_boxctl_dir(boxctl_dir, project_dir)
    ...
    _save_packages_config(boxctl_dir, packages)
    ...
    if pctx.manager.container_exists(pctx.container_name):
        if not _warn_if_agents_running(pctx.manager, pctx.container_name, "container rebuild"):
            ...
            console.print("[blue]Run 'boxctl rebase' when ready to install[/blue]")
            return
...
@handle_errors
def packages_init():
    """Initialize packages section in .boxctl/config.yml with empty configuration."""
    project_dir = resolve_project_dir()
    boxctl_dir = get_boxctl_dir(project_dir)
    _require_boxctl_dir(boxctl_dir, project_dir)

    config_path = project_dir / ".boxctl/config.yml"
    config = ProjectConfig(project_dir)
    if "packages" in config.config:
        console.print("[yellow]packages already configured in .boxctl/config.yml[/yellow]")
        return
    ...
    console.print("[green]✓ Initialized packages in .boxctl/config.yml[/green]")
