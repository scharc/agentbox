...
the BOXCTL_PROJECT_DIR environment variable check.

For container name resolution, use boxctl.container_naming module directly.
"""
...
from boxctl.container_naming import (
    resolve_project_dir,
...
def find_project_by_container(container_name: str) -> Optional[Path]:
    ...
    Args:
        container_name: Container name (e.g., boxctl-steuerung)
    ...
    for base in search_paths:
        ...
        if candidate.exists() and ProjectPaths.boxctl_dir(candidate).exists():
            return candidate
        # Try case-insensitive search
        try:
            for d in base.iterdir():
                if d.is_dir() and d.name.lower() == project_name.lower():
                    if ProjectPaths.boxctl_dir(d).exists():
                        return d
...
def get_boxctl_dir(project_dir: Optional[Path] = None) -> Path:
    """Get the .boxctl directory for a project.

    Args:
        project_dir: Optional project directory (resolved if not provided)

    Returns:
        Path to .boxctl directory
    """
    return ProjectPaths.boxctl_dir(resolve_project_dir(project_dir))
...
def get_config_file(project_dir: Optional[Path] = None) -> Path:
    ...
    Returns:
        Path to .boxctl/config.yml file
    """
    return ProjectPaths.config_file(resolve_project_dir(project_dir))


def is_initialized(project_dir: Optional[Path] = None) -> bool:
    """Check if a project directory is initialized for boxctl.

    A project is considered initialized if it has a .boxctl directory.
    ...
    """
    return get_boxctl_dir(project_dir).exists()
