...
"""Configuration management for boxctl projects."""
...
class ProjectConfig:
    """Manages boxctl configuration for projects.

    Config location: .boxctl/config.yml
    Uses Pydantic model as single source of truth. No fallback to raw dict.
    """

    # Config path inside .boxctl directory
    CONFIG_PATH = f"{ProjectPaths.BOXCTL_DIR_NAME}/{ProjectPaths.CONFIG_NAME}"
    ...
    def save(self, quiet: bool = False) -> None:
        ...
        try:
            # Ensure parent directory exists (for new .boxctl/ location)
    ...
    @property
    def boxctl_version(self) -> Optional[str]:
        """Get the boxctl version that created/last updated this config."""
        return self._model.boxctl_version if self._model else None

    @boxctl_version.setter
    def boxctl_version(self, value: str) -> None:
        """Set the boxctl version."""
        self._ensure_model().boxctl_version = value

    def is_version_outdated(self) -> bool:
        """Check if the config was created with an older boxctl version."""
        stored_version = self.boxctl_version
        if stored_version is None:
            return False
        return stored_version != BOXCTL_VERSION
    ...
    def create_template(self) -> None:
        """Create a template config.yml file in .boxctl/ directory."""
        if self.exists():
