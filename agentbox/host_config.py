"""Centralized host-side configuration for Agentbox."""
...
class HostConfig:
    ...
    def agentbox_dir(self) -> Path:
        ...
        Priority:
        1. AGENTBOX_DIR environment variable
        2. Package installation location
        3. ~/.local/share/agentbox (fallback)
        """
        # Check config override
        if self._model:
            config_dir = self._model.paths.agentbox_dir
            if config_dir:
                return Path(config_dir)
        else:
            config_dir = self._config.get("paths", {}).get("agentbox_dir")
            if config_dir:
                return Path(config_dir)

        # 1. Environment variable (highest priority)
        env_dir = os.getenv("AGENTBOX_DIR")
        if env_dir:
            return Path(env_dir)
        ...
        try:
            ...
            pkg_path = Path(agentbox.__file__).parent.parent
