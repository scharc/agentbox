"""Unified logging and debug infrastructure for Agentbox.

This module provides:
1. Centralized logging configuration
2. Debug mode via AGENTBOX_DEBUG env var or programmatic flag
3. Log levels via AGENTBOX_LOG_LEVEL env var
4. Dual output: Rich console for CLI, file logging for debugging
5. Daemon mode: stderr-only for background processes

Usage:
    from agentbox.utils.logging import get_logger, configure_logging
...
Environment Variables:
    AGENTBOX_DEBUG=1        Enable debug mode (verbose output)
    AGENTBOX_LOG_LEVEL=DEBUG  Set log level (DEBUG, INFO, WARNING, ERROR)
    AGENTBOX_LOG_FILE=/path  Override log file location
"""
...
def _get_log_dir() -> Path:
    ...
    env_log_file = os.environ.get("AGENTBOX_LOG_FILE")
    if env_log_file:
        return Path(env_log_file).parent

    # Default to .agentbox/logs in workspace or home
    workspace = Path(os.environ.get("AGENTBOX_PROJECT_DIR", Path.cwd()))
    agentbox_dir = workspace / ".agentbox"

    # Fall back to home directory if workspace doesn't have .agentbox
    if not agentbox_dir.exists():
        agentbox_dir = Path.home() / ".agentbox"

    log_dir = agentbox_dir / "logs"
...
def _get_log_file() -> Path:
    ...
    if env_log_file:
        ...
        _log_file = _get_log_dir() / "agentbox.log"
...
def is_debug_mode() -> bool:
    ...
    return _debug_mode or os.environ.get("AGENTBOX_DEBUG", "").lower() in ("1", "true", "yes")
...
def configure_logging(
    ...
    if log_level:
        ...
        level_name = os.environ.get("AGENTBOX_LOG_LEVEL", "DEBUG" if _debug_mode else "INFO").upper()

    level = getattr(logging, level_name, logging.INFO)

    # Get root agentbox logger
    root_logger = logging.getLogger("agentbox")
...
class AgentboxLogger:
    ...
    def debug(self, message: str, console_output: bool = False) -> None:
        ...
        or enable AGENTBOX_DEBUG to see in console.
...
def get_logger(name: str) -> AgentboxLogger:
    ...
    # Ensure name is under agentbox namespace
    if not name.startswith("agentbox"):
        name = f"agentbox.{name}"

    return AgentboxLogger(name)


# Convenience function for daemon processes
def get_daemon_logger(name: str) -> AgentboxLogger:
    ...
    Returns:
        AgentboxLogger configured for daemon output
    """
...
def log_startup_info() -> None:
    ...
    logger = get_logger("agentbox.startup")
    ...
    for var in ["AGENTBOX_DEBUG", "AGENTBOX_LOG_LEVEL", "AGENTBOX_PROJECT_DIR"]:
