...
class TmuxStreamSession:
    ...
    async def start(self):
        ...
        self._pipe_pane_proc = subprocess.Popen(
            ["sh", "-c", f"tail -f {self.input_fifo} | /usr/bin/tmux load-buffer -b stream-input - && /usr/bin/tmux paste-buffer -b stream-input -t {self.session_name}"],
    ...
    async def cleanup(self):
        ...
        # Stop tmux pipe-pane
        subprocess.run([
            "/usr/bin/tmux", "pipe-pane", "-t", self.session_name
    ...
    def __del__(self):
        ...
        if not self._cleanup_done:
            ...
            import subprocess
            subprocess.run(["/usr/bin/tmux", "pipe-pane", "-t", self.session_name],
                         check=False, capture_output=True)
