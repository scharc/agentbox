...
"""FastAPI server for agentbox web UI - HOST-SIDE VERSION.
...
agentbox containers and their sessions.
"""
...
from agentbox.web.tmux_manager import (
    get_all_sessions,
...
# Get workspace root from this file's location (agentbox/web/host_server.py)
...
# Create FastAPI app
app = FastAPI(title="Agentbox Web UI (Host)", version="0.2.0", lifespan=lifespan)
...
async def log_mobile_debug(log: DebugLog):
    ...
    # Write to workspace .agentbox directory (shared with container)
    debug_file = WORKSPACE_ROOT / ".agentbox" / "mobile-debug.log"
...
async def restart_service():
    """Restart the agentboxd service via systemctl."""
    import subprocess
...
async def rebase_container(container_name: str):
    ...
    try:
        ...
        project_name = container_name.replace("agentbox-", "")
...
if __name__ == "__main__":
    ...
    port = int(os.getenv("AGENTBOX_WEB_PORT", str(default_port)))

    # Environment variable can override hosts (comma-separated)
    env_hosts = os.getenv("AGENTBOX_WEB_HOST")
    if env_hosts:
    ...
    async def serve_all():
        servers = []
        for host in hosts:
            logger.info(f"Starting Agentbox Web UI (Host) on {host}:{port}")
