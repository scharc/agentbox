...
def _get_agentboxd_socket_path() -> Path:
    """Get the agentboxd IPC socket path."""
    import os
...
def _send_agentboxd_command(command: dict) -> dict:
    ...
    if not socket_path.exists():
        return {"ok": False, "error": "agentboxd not running"}
...
class PortConflict:
    ...
    blocker_type: str  # "agentbox" or "external"
    blocker_container: Optional[str] = None  # if agentbox
...
def check_configured_ports(
    ...
    for port_spec in config.ports_host:
        ...
        if result.get("available") is False:
            ...
            if used_by.get("type") == "agentbox" and used_by.get("container") == container_name:
                continue
...
def format_conflict_message(conflict: PortConflict) -> str:
    ...
    if conflict.blocker_type == "agentbox":
        ...
        return (
            ...
            f"agentbox: [cyan]{display_name}[/cyan]"
        )
    else:
...
def release_port_from_container(container_name: str, port: int, direction: str) -> bool:
    """Release a port from another agentbox container.
