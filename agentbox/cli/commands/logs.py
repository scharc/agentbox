...
def _complete_session_name(ctx, param, incomplete):
    ...
    map_file = Path.cwd() / '.agentbox' / 'session-map.json'
...
def get_agentbox_dir() -> Path:
    """Get .agentbox directory for current project."""
    return get_project_dir() / '.agentbox'


def load_session_map() -> dict:
    """Load session mapping."""
    map_file = get_agentbox_dir() / 'session-map.json'
    if map_file.exists():
        try:
...
def logs_export(session: Optional[str], output: Optional[str]):
    ...
    if not session:
        ...
        for s in data.keys():
            console.print(f"  {s}")
        console.print("\nUsage: agentbox logs export <session>")
        return

    if session not in data:
        console.print(f"[red]Session not found: {session}[/red]")
        console.print("Use 'agentbox logs list' to see available sessions")
        return
    ...
    history_dir = get_agentbox_dir() / 'history'
    ...
    if output:
        ...
        logs_dir = get_agentbox_dir() / 'logs'
...
def logs_show(session: Optional[str], limit: int):
    ...
    if not session:
        ...
        for s in data.keys():
            console.print(f"  {s}")
        console.print("\nUsage: agentbox logs show <session>")
        return
