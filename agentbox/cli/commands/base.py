...
@cli.group()
def base():
    """Manage the Agentbox base Docker image."""
    pass
...
@handle_errors
def rebuild():
    """Rebuild the agentbox-base:latest Docker image."""
    # Find Dockerfile.base
    package_root = Path(__file__).resolve().parents[3]
    if not (package_root / "Dockerfile.base").is_file():
        raise click.ClickException(
            "Dockerfile.base not found. Run from the Agentbox repo or build manually:\n"
    ...
    try:
        result = subprocess.run(
            [
                ...
                "--build-arg",
                f"AGENTBOX_VERSION={AGENTBOX_VERSION}",
                str(package_root),
            ],
            check=False,
        )
