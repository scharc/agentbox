...
"""Quick access command - mobile-friendly TUI for agentbox."""
...
from agentbox.cli.commands.mcp import mcp_add, _add_mcp, _get_installed_mcps
...
def get_system_status() -> Optional[dict]:
    """Fetch system status from the agentboxd web API.
...
def status_menu() -> Optional[str]:
    ...
    from agentbox.config import parse_port_spec
    ...
    if status is None:
        ...
        console.print("[dim]Run 'agentboxd' on the host to start the service[/dim]")
        console.print()
        console.print("[dim]─" * 30 + "[/dim]")
    ...
    console.print(f"  [green]●[/green] agentboxd running")
    console.print()
...
def new_session_menu(container_data: dict) -> Optional[str]:
    ...
    if project_path:
        os.environ["AGENTBOX_PROJECT_DIR"] = project_path
...
def get_added_mcps(project_path: str) -> set:
    ...
    from agentbox.cli.helpers import _get_project_context
    ...
    try:
        ...
        old_dir = os.environ.get("AGENTBOX_PROJECT_DIR")
        os.environ["AGENTBOX_PROJECT_DIR"] = project_path
        pctx = _get_project_context()
        result = _get_installed_mcps(pctx)
        if old_dir:
            os.environ["AGENTBOX_PROJECT_DIR"] = old_dir
        return result
    except Exception:
        return set()
    ...
    if choice.isalpha() and len(choice) == 1:
        ...
        if 0 <= idx < len(all_items):
            ...
            if mcp_name in added_mcps:
                ...
                from agentbox.cli.helpers import _get_project_context, _rebuild_container
                try:
                    pctx = _get_project_context()
...
def get_added_skills(project_path: str) -> set:
    ...
    from agentbox.utils.project import get_agentbox_dir

    if not project_path:
        return set()

    agentbox_dir = get_agentbox_dir(Path(project_path))
    return _get_installed_skills(agentbox_dir)
    ...
    if choice.isalpha() and len(choice) == 1:
        ...
        if 0 <= idx < len(all_items):
            ...
            if sname in added_skills:
                ...
                try:
                    agentbox_dir = get_agentbox_dir(Path(project_path))
                    if _add_skill(sname, lib, agentbox_dir):
...
def network_menu(container_data: dict) -> Optional[str]:
    ...
    manager = ContainerManager()
    all_containers = manager.get_all_containers(include_agentbox=False)
...
def new_container_flow() -> Optional[str]:
    ...
    os.environ["AGENTBOX_PROJECT_DIR"] = selected_path

    # Check if already initialized
    agentbox_dir = Path(selected_path) / ".agentbox"
    already_initialized = agentbox_dir.exists()
...
def main_menu() -> Optional[str]:
    ...
    render_menu("AGENTBOX QUICK", sections, actions)
...
@cli.command("quick")
def quick():
    """Quick access menu - mobile-friendly TUI for agentbox.
