...
from agentbox.cli.helpers.completions import _complete_docker_containers, _complete_connected_containers
...
def available(show_all: Optional[str]):
    ...
    Pass 'all' to include agentbox containers.

    Examples:
        abox network available      # Non-agentbox containers only
        abox network available all  # All containers
    """
    manager = ContainerManager()
    include_agentbox = show_all == "all"
    ...
    console.print("\n[blue]Connect with:[/blue] agentbox network connect <name>")
...
def list():
    ...
    # Load connections
    connections = _load_containers_config(pctx.agentbox_dir)

    if not connections:
        console.print("[yellow]No container connections configured[/yellow]")
        console.print("\n[blue]Connect to a container with:[/blue] agentbox network connect <name>")
        return
    ...
    if warnings:
        ...
        console.print("\n[blue]Remove with:[/blue] agentbox network disconnect <name>")
...
def connect(target_container: str):
    """Connect agentbox to a running container's network.
    ...
    # Ensure Agentbox container is running
    if not pctx.manager.container_exists(pctx.container_name):
        raise click.ClickException(
            f"Agentbox container {pctx.container_name} not found. Start it with: agentbox start"
        )

    if not pctx.manager.is_running(pctx.container_name):
        console.print(f"[blue]Starting Agentbox container {pctx.container_name}...[/blue]")
    ...
    try:
        ...
        containers = pctx.manager.get_all_containers(include_agentbox=False)
    ...
    if not already_connected:
        ...
        # Save connections
        _save_containers_config(pctx.agentbox_dir, connections)
