...
def _get_unified_mcp_path(agentbox_dir: Path) -> Path:
    """Get path to unified mcp.json."""
    return agentbox_dir / "mcp.json"
...
def _get_installed_mcps(pctx) -> Set[str]:
    ...
    mcp_path = _get_unified_mcp_path(pctx.agentbox_dir)
    if mcp_path.exists():
        try:
...
def _add_mcp(name: str, lib_manager: LibraryManager, pctx) -> tuple[bool, bool]:
    ...
    unified_mcp_path = _get_unified_mcp_path(pctx.agentbox_dir)
...
def mcp_add(name: str):
    ...
    console.print(f"[green]✓ Added '{name}' to .agentbox/mcp.json[/green]")
...
def mcp_remove(name: str):
    ...
    console.print(f"[green]✓ Removed '{name}' from .agentbox/mcp.json[/green]")
...
MCP_INIT_PROMPT = """Analyze this MCP server folder and generate a config.json for agentbox.
...
Important paths:
- MCP folder in container: /home/abox/.config/agentbox/mcp/MCP_NAME
...
def mcp_init(path: str | None, agent: str):
    ...
    # 2. AGENTBOX_PROJECT_DIR if set (shell wrapper sets this to actual CWD)
    # 3. Fall back to os.getcwd()
    if path:
        mcp_path = Path(path)
    elif os.environ.get("AGENTBOX_PROJECT_DIR"):
        mcp_path = Path(os.environ["AGENTBOX_PROJECT_DIR"])
    else:
        mcp_path = Path.cwd()
    ...
    # Collect relevant files (skip agentbox-specific files)
    relevant_files = [
        "pyproject.toml",
        "package.json",
        "server.json",
    ...
    skip_dirs = {".git", ".agentbox", "__pycache__", ".pytest_cache", "node_modules", ".venv", "venv", ".tox", ".mypy_cache", ".ruff_cache"}
    dir_listing = []
