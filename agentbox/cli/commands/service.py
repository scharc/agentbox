...
"""Service management commands for agentboxd (notifications + web UI)."""
...
def _get_service_unit_path() -> Path:
    ...
    return systemd_dir / "agentboxd.service"


def _get_config_path() -> Path:
    """Get path to agentboxd config file."""
    return Path.home() / ".config" / "agentbox" / "config.yml"
...
def _create_service_file() -> str:
    ...
    # Find agentbox executable
    agentbox_path = shutil.which("agentbox") or shutil.which("abox")
    if not agentbox_path:
        agentbox_path = "/usr/bin/env agentbox"

    return f"""[Unit]
Description=Agentbox Daemon (Notifications + Web UI)
...
Environment=AGENTBOXD_SOCKET={socket_path}
ExecStart={agentbox_path} service serve {socket_path}
Restart=on-failure
RestartSec=5
...
def _create_default_config() -> str:
    ...
    return """# Agentbox Daemon Configuration
...
@cli.group()
def service():
    """Manage agentboxd service (notifications + web UI)."""
    pass
...
def service_install():
    ...
    try:
        ...
        subprocess.run(["systemctl", "--user", "enable", "agentboxd"], check=False, timeout=SYSTEMCTL_TIMEOUT)
        subprocess.run(["systemctl", "--user", "start", "agentboxd"], check=False, timeout=SYSTEMCTL_TIMEOUT)
...
def service_uninstall():
    ...
    try:
        subprocess.run(["systemctl", "--user", "stop", "agentboxd"], check=False, timeout=SYSTEMCTL_TIMEOUT)
        subprocess.run(["systemctl", "--user", "disable", "agentboxd"], check=False, timeout=SYSTEMCTL_TIMEOUT)
...
def service_start():
    """Start the service."""
    try:
        subprocess.run(["systemctl", "--user", "start", "agentboxd"], check=True, timeout=SYSTEMCTL_TIMEOUT)
...
def service_stop():
    """Stop the service."""
    try:
        subprocess.run(["systemctl", "--user", "stop", "agentboxd"], check=True, timeout=SYSTEMCTL_TIMEOUT)
...
def service_restart():
    """Restart the service."""
    try:
        subprocess.run(["systemctl", "--user", "restart", "agentboxd"], check=True, timeout=SYSTEMCTL_TIMEOUT)
...
def service_status():
    """Show service status."""
    try:
        subprocess.run(["systemctl", "--user", "status", "agentboxd"])
    except FileNotFoundError:
        raise click.ClickException("systemctl not found")
...
def service_logs(lines: str):
    ...
    cmd = ["journalctl", "--user", "-u", "agentboxd", "-n", str(num_lines)]
...
def service_follow():
    ...
    cmd = ["journalctl", "--user", "-u", "agentboxd", "-f"]
...
def service_serve(socket_path: str = None):
    """Run agentboxd in foreground for debugging.
    ...
    """
    from agentbox.agentboxd import run_agentboxd
    run_agentboxd(socket_path)
