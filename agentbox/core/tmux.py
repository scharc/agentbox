...
from agentbox.utils.exceptions import TmuxError
...
def capture_pane(
    ...
    socket_path = get_tmux_socket_path(manager, container_name)
    tmux_cmd = ["/usr/bin/tmux", "capture-pane", "-t", session_name, "-p", "-S", f"-{lines}"]
    if socket_path:
        tmux_cmd = ["/usr/bin/tmux", "-S", socket_path, "capture-pane", "-t", session_name, "-p", "-S", f"-{lines}"]
...
def send_keys(
    ...
    socket_path = get_tmux_socket_path(manager, container_name)
    tmux_cmd = ["/usr/bin/tmux", "send-keys", "-t", session_name]
    if literal:
        tmux_cmd.append("-l")
    tmux_cmd.append(keys)

    if socket_path:
        tmux_cmd = ["/usr/bin/tmux", "-S", socket_path] + tmux_cmd[1:]
...
def resize_window(
    ...
    socket_path = get_tmux_socket_path(manager, container_name)
    tmux_cmd = ["/usr/bin/tmux", "resize-window", "-t", session_name, "-x", str(width), "-y", str(height)]
    if socket_path:
        tmux_cmd = ["/usr/bin/tmux", "-S", socket_path] + tmux_cmd[1:]
