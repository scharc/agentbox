...
This module handles all container name resolution for agentbox:
...
def generate_default_name(project_dir: Path) -> str:
    ...
    Returns:
        Default container name (agentbox-<sanitized_name>)
    """
...
def generate_hashed_name(project_dir: Path) -> str:
    ...
    Returns:
        Container name with hash suffix (agentbox-<name>-<hash>)
    """
...
def extract_project_name(container_name: str) -> Optional[str]:
    ...
    Handles both regular names (agentbox-myapp) and hashed names (agentbox-myapp-a1b2).

    Args:
        container_name: Full container name

    Returns:
        Project name portion, or None if not an agentbox container
    """
